...
boolean load_wifi() {
  ...
  topic_led = (const char*)WIFI["Topic_LED"];
  topic_all = (const char*)WIFI["Topic_ALL"];
...
boolean setup_wifi() {
  ...
  for (int i = 0; i < maxtry; i++) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      LED.fill(LED.Color(0, 5, 0, 0), 0, NUM_LEDS - 1);
      LED.show();
      //fill_solid(LED, NUM_LEDS, CRGB( 0, 5, 0));
      //FastLED.show();
      return (true);
    }
    LED.fill(LED.Color(5, 0, 0, 0), 0, NUM_LEDS);
    LED.show();
    //fill_solid(LED, NUM_LEDS, CRGB( 5, 0, 0));
    //FastLED.show();
    delay(250);
    Serial.print(".");
    LED.fill(LED.Color(0, 0, 0, 0), 0, NUM_LEDS);
    LED.show();
  ...
  //fill_solid(LED, NUM_LEDS, CRGB( 5, 0, 0));
  //FastLED.show();
  LED.fill(LED.Color(5, 0, 0, 0), 0, NUM_LEDS - 1);
  LED.show();
...
void startWebServer() {
  ...
  webServer.onNotFound([]() {
    ...
    s += "</select><br>Topic LED: <input name=\"tcmd1\" length=64 type=\"text\">";
    s += "</select><br>Topic ALL: <input name=\"tcmd2\" length=64 type=\"text\">";
  ...
  webServer.on("/setap", []() {
    ...
    if (urlDecode(webServer.arg("tcmdservo")) != "") {
      WIFI["Topic_LED"] = urlDecode(webServer.arg("tcmd1"));
    }
    if (urlDecode(webServer.arg("tcmdservo")) != "") {
      WIFI["Topic_ALL"] = urlDecode(webServer.arg("tcmd2"));
    }
